
import React, { useState } from 'react';

interface RatingFilterProps {
  selectedRating: number | null;
  onRatingChange: (rating: number | null) => void;
}

const StarIcon = ({ filled, isHovered }: { filled: boolean; isHovered?: boolean }) => (
    <svg 
        className={`h-8 w-8 cursor-pointer transition-colors duration-200 
            ${filled ? 'text-cyan-400' : 'text-gray-600'} 
            ${isHovered ? 'text-cyan-300' : ''}`
        }
        fill="currentColor" 
        viewBox="0 0 20 20"
    >
        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
    </svg>
);

const RatingFilter: React.FC<RatingFilterProps> = ({ selectedRating, onRatingChange }) => {
    const [hoverRating, setHoverRating] = useState<number | null>(null);
    
    const handleStarClick = (rating: number) => {
        if (selectedRating === rating) {
            onRatingChange(null); // Deselect if clicked again
        } else {
            onRatingChange(rating);
        }
    };

    return (
        <div className="mb-8 flex flex-col items-center justify-center">
             <p className="text-lg text-gray-300 mb-3 font-semibold">Filter by rating:</p>
            <div className="flex items-center space-x-2">
                {[1, 2, 3, 4, 5].map((star) => (
                    <button
                        key={star}
                        aria-label={`Filter by ${star} stars or more`}
                        onClick={() => handleStarClick(star)}
                        onMouseEnter={() => setHoverRating(star)}
                        onMouseLeave={() => setHoverRating(null)}
                        className="focus:outline-none focus:ring-2 focus:ring-cyan-500 rounded-full p-1"
                    >
                        <StarIcon 
                            filled={star <= (hoverRating || selectedRating || 0)}
                        />
                    </button>
                ))}
            </div>
        </div>
    );
};

export default RatingFilter;